...
function shouldProcessMessage(message, state, firstRunAt) {
  ...
  if (message.msg_type !== "text") return false;
  ...
  const ignoreBeforeMs = Number(state.ignoreBeforeMs || firstRunAt - initialLookbackMinutes * 60_000);
  ...
  return {
    ...
    event: {
      ...
      message: {
        ...
        content: JSON.stringify({ text: String(message.content || "") }),
      },
    },
  };
}
...
async function forward(message) {
  ...
  console.error(`[feishu-poller] forwarded message_id=${message.message_id} status=${response.status}`);
}
